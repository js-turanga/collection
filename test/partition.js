...
describe( "partition -", function() {
    ...
    it( 'should also work when collection is based on an object', function() {

        const collection = collect( {
            name: 'Sadio Mané', number: 19, club: 'Liverpool FC',
        } );

        const arr = collection.partition( v => typeof v === 'string' );

        expect( arr.all() ).to.eql( [
            collect( { name: 'Sadio Mané', club: 'Liverpool FC', } ),
            collect( { number: 19 } ),
        ] );
    } );
} );
