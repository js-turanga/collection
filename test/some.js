...
describe( "some -", function() {
    ...
    it( 'should work with an collection with an array of objects', function() {

        const collection = collect( [
            { name: 'Steven Gerrard', number: 8, }, 
            { name: 'Steve Jobs', number: 2, }
        ] );
    ...
    it( 'should return whether the collection contains a given key', function() {
        ...
        expect( collection.some( 'name', 'Mohamed Salah' ) ).to.eql( true );
        expect( collection.some( 'number' ) ).to.eql( true );
        expect( collection.some( 'number', 11 ) ).to.eql( true );
    } );
} );
